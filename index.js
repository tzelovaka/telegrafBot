...
bot.on('text', async (ctx, next) => {
  ...
  if (!u) await ctx.replyWithHTML(`Добро пожаловать в <b>Сторинтер</b>! Данная система разработана для <i>создания</i> и <i>чтения</i> интерактивных историй (в них сюжет зависит от <i>выбора/выборов</i> читателя в процессе чтения). Если вы хотите попробовать прочитать историю: 
  /start -> "Популярное", 
  если создать: 
  "Редактор" <i>(большая голубая кнопка слева)</i> – для создания <b>именно сюжета</b> выйдите из главного меню путём нажатия красной кнопки справа сверху.
  Удачи!`)
